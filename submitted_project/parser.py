...
class Config:
    """
    Store the validated configuration values used to generate a maze.
    """
    width: int
    height: int
...
def read_config_file(read_file_path: str) -> dict[str, str]:
    ...
    Ignores empty lines and comments, and stores each KEY=VALUE pair
    in a dictionary.
    """
...
def parse_config(file_path: str) -> Config:
    ...
    Validates the required keys, maze dimensions, entry and exit
    coordinates, seed, perfect-maze option, and output file.
    """
